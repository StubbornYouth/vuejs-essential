  ...
  <div class="col-md-9 left-col pull-right">
        ...
        <div class="voted-users">
          <div class="user-lists">
            <span v-for="likeUser in likeUsers">
              ...
              <!-- <img :src="user && user.avatar" class="img-thumbnail avatar avatar-middle" :class="{ 'animate__animated animate__swing' : likeUser.uid === 1 }"> -->
              <router-link :to="`/${likeUser.uname}`" :src="likeUser.uavatar" tag="img" class="img-thumbnail avatar avatar-middle" :class="{ 'animate__animated animate__swing' : likeUser.uid === 1 }"></router-link> 
            </span>
          </div>
    ...
    <div class="replies panel panel-default list-panel replies-index">
      ...
      <div class="panel-body">
        <!-- <transition-group> 可用于列表的过渡，但它内部的元素必须具有唯一的 key 属性，<transition-group> 会以一个真实元素呈现，默认为一个 <span>，我们可以使用 tag 特性将其更改为其他元素 -->
        <!-- 使用 key 可以使 Vue 跟踪每个节点的身份，从而重用和重新排序现有元素，其值应该是唯一的 -->
...
export default {
  ...
  created() {
    ...
    // 能从当前路由对象获取 articleId，是因为我们在仓库 post 事件最后面的导航中传递了该参数
  ...
  methods: {
    ...
    renderComments(comments) {
      if (Array.isArray(comments)) {
        ...
        // 深拷贝 comments 是为了不影响已保存的文章，其方法等价于：
        // const newComments = comments.map(function (comment) {
        //   return Object.assign({}, comment)
        // })
        // 前面的方法只处理了对象的第一层数据，当对象能被 JSON 解析时，可以使用下面的方法进行完整的深拷贝：
...
<style scoped>
.markdown-body /deep/ .hljs { background:unset; ; color: #fff;}
