...
export default {
  ...
  // 举例来说，当路由的查询参数由 '?q=foo' 变成 '?q=bar' 时，Search.vue 是被复用的，beforeRouteEnter 不会被再次调用，我们需要在 beforeRouteUpdate 中根据路由参数的变化，重新获取对应数据，需要使用 next() 确认导航。
  // 当然，我们也可以通过监听 '$route' 来代替 beforeRouteUpdate
